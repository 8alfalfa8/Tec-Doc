...
- [✅ 目的](#目的)
- [📁 ブランチ戦略と構成（親ブランチ含む）](#ブランチ戦略と構成親ブランチ含む)
- [🔄 ブランチ間の従属関係（構成図）](#ブランチ間の従属関係構成図)
- [🛠️ 開発・マージフロー（例：featureブランチ）](#開発マージフロー例featureブランチ)
- [🚨 リリース・障害対応フロー](#リリース障害対応フロー)
  - [📦 リリースフロー（release/\*）](#リリースフローrelease)
  - [🔥 ホットフィックス（hotfix/\*）](#ホットフィックスhotfix)
- [✅ マージルール（レビュー・保護ルール）](#マージルールレビュー保護ルール)
- [🧷 補足：権限・保護ルールの設定（GitHub）](#補足権限保護ルールの設定github)
  - [🔐 必須設定（リポジトリ > Settings > Branches）](#必須設定リポジトリ-settings-branches)
  - [👥 権限ロールの使い分け（GitHubチーム）](#権限ロールの使い分けgithubチーム)
- [⚙️ CI/CDとの連携（GitHub Actions例）](#cicdとの連携github-actions例)
- [📝 運用ポイント](#運用ポイント)
  - [✅ **`hotfix/*` の修正を `release/*` にも反映したい場合**](#hotfix-の修正を-release-にも反映したい場合)
- [🔁 運用フロー（複数マージ先）](#運用フロー複数マージ先)
- [🎯 マージ先を選ぶポイント](#マージ先を選ぶポイント)
- [💡 注意点：`release/*` への反映が必要なケース](#注意点release-への反映が必要なケース)
- [🛠 例：実際のGit操作イメージ](#例実際のgit操作イメージ)
- [✅ 結論：hotfix の反映先は3箇所になる場合がある](#結論hotfix-の反映先は3箇所になる場合がある)
<!-- TOC_END -->
