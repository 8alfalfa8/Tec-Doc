...
- [✅ 1. 概要：AWS基盤マイクロサービス認証システムとは](#1-概要aws基盤マイクロサービス認証システムとは)
- [✅ 2. 主な構成要素（全体構成図）](#2-主な構成要素全体構成図)
- [✅ 3. 認証・認可のパターン](#3-認証認可のパターン)
- [✅ 4. AWSサービス利用の設計ポイント](#4-awsサービス利用の設計ポイント)
  - [① Amazon Cognito](#①-amazon-cognito)
  - [② API Gateway](#②-api-gateway)
  - [③ Lambda（カスタム認証処理用）](#③-lambdaカスタム認証処理用)
  - [④ IAM + STS](#④-iam-sts)
- [✅ 5. 認証フロー詳細（Cognito + OIDC）](#5-認証フロー詳細cognito-oidc)
  - [① サインインフロー](#①-サインインフロー)
  - [② Tokenの検証フロー](#②-tokenの検証フロー)
- [✅ 6. IAMベースのサービス間認証](#6-iamベースのサービス間認証)
  - [利用シナリオ：](#利用シナリオ)
  - [方法：](#方法)
- [✅ 7. セキュリティ・運用観点の設計](#7-セキュリティ運用観点の設計)
- [✅ 8. 拡張可能な実装例：認証ゲートウェイ構築](#8-拡張可能な実装例認証ゲートウェイ構築)
- [✅ 9. TerraformやCDKでのIaC化の例（Cognito）](#9-terraformやcdkでのiac化の例cognito)
- [✅ AWSマイクロサービス環境における認証の全体像](#awsマイクロサービス環境における認証の全体像)
  - [✅ ① ユーザー認証（エンドユーザー → API）](#①-ユーザー認証エンドユーザー-api)
    - [💡 目的](#目的)
    - [💡 一般構成](#一般構成)
    - [🔧 技術・構成詳細](#技術構成詳細)
    - [🛡️ セキュリティ対策](#セキュリティ対策)
  - [✅ ② API Gatewayとマイクロサービスの認証](#②-api-gatewayとマイクロサービスの認証)
    - [💡 目的](#目的)
    - [💡 一般構成](#一般構成)
    - [🔧 技術選択肢と構成例](#技術選択肢と構成例)
    - [🛡️ ベストプラクティス](#ベストプラクティス)
  - [✅ ③ マイクロサービス間の認証（Service-to-Service）](#③-マイクロサービス間の認証service-to-service)
    - [💡 目的](#目的)
    - [💡 一般構成](#一般構成)
    - [🔧 主な方式](#主な方式)
  - [✅ サンプル構成例（IAM＋Cognito統合パターン）](#サンプル構成例iamcognito統合パターン)
  - [✅ 監査・セキュリティ設計補足](#監査セキュリティ設計補足)
  - [✅ 最後に：設計ガイドラインまとめ](#最後に設計ガイドラインまとめ)
- [✅ マイクロサービス側の認証最小化](#マイクロサービス側の認証最小化)
  - [✅ 目指す構成の要点](#目指す構成の要点)
  - [✅ 推奨構成アーキテクチャ（共通認証カプセル化）](#推奨構成アーキテクチャ共通認証カプセル化)
  - [✅ 実現するための構成要素と設計ポイント](#実現するための構成要素と設計ポイント)
    - [① **API Gateway + Cognitoオーソライザー**](#①-api-gateway-cognitoオーソライザー)
    - [② **API Gatewayでの認証責任の完結**](#②-api-gatewayでの認証責任の完結)
    - [③ **各マイクロサービスの最小構成**](#③-各マイクロサービスの最小構成)
  - [✅ オプション：サービスメッシュとmTLSによる“ゼロトラスト対応”](#オプションサービスメッシュとmtlsによるゼロトラスト対応)
  - [✅ 補足：Lambda関数を共通Proxyとする案（認証抽象化）](#補足lambda関数を共通proxyとする案認証抽象化)
  - [✅ 実現時の注意点](#実現時の注意点)
  - [✅ まとめ：理想的な共通認証構成](#まとめ理想的な共通認証構成)
- [✅ ステップ①：要件定義](#ステップ①要件定義)
- [✅ ステップ②：設計](#ステップ②設計)
- [✅ ステップ③：実装](#ステップ③実装)
- [✅ ステップ④：セキュリティ](#ステップ④セキュリティ)
- [✅ ステップ⑤：運用](#ステップ⑤運用)
- [✅ まとめ表（全体一覧）](#まとめ表全体一覧)
<!-- TOC_END -->
