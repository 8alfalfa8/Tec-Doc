...
# ◆ 単一物理データベスを使用するマイクロサービス構成設計書

## 1. 概要

### 1.1. 設計方針
単一物理データベースを使用しながらも、マイクロサービスの設計原則（サービス自律性、疎結合性）を維持するための構成設計です。

### 1.2. 適用条件
- 開発初期段階または小規模システム
- コスト制約がある環境
- 段階的なマイグレーションを計画している場合
...
CREATE ROLE product_service_role;
...
```yaml
# 各マイクロサービスのデータベース接続設定
user-service:
  datasource:
    url: jdbc:postgresql://localhost:5432/company_db?currentSchema=user_service
    username: user_service_user
    password: ${USER_SERVICE_DB_PASSWORD}

order-service:
  datasource:
    url: jdbc:postgresql://localhost:5432/company_db?currentSchema=order_service
...
-- 他のスキーマへのアクセス禁止
REVOKE ALL PRIVILEGES ON SCHEMA user_service FROM order_service_user;
REVOKE ALL PRIVILEGES ON SCHEMA order_service FROM user_service_user;
```

### 3.2. データアクセス制約
- 各サービスは自身のスキーマのみアクセス可能
- クロススキーマアクセスはAPI経由のみ許可
- データベースレベルでの外部キー制約は禁止
...
CREATE TABLE order_service.orders (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL, -- 参照のみ、外部キー制約なし
...
```mermaid
graph LR
    A[Order Service] -->|API Call| B[User Service]
    C[Product Service] -->|API Call| D[Order Service]
```

### 5.2. データ参照パターン
```java
// OrderServiceがUserServiceのデータを参照する例
@Service
...
### 8.1. 段階的移行戦略
1. **フェーズ1**: 単一DB内でのスキーマ分離
2. **フェーズ2**: サービスごとの論理的分離の徹底
3. **フェーズ3**: 物理的分離の準備（接続設定の抽象化）
4. **フェーズ4**: 必要に応じた物理的分離の実施
...
spring:
  datasource:
    url: ${DB_URL:jdbc:postgresql://localhost:5432/company_db}
    schema: ${SERVICE_SCHEMA:user_service}
```
...
### 10.1. 設計原則
1. **スキーマ境界の厳守**: サービスは自身のスキーマのみ操作
2. **APIファースト**: サービス間通信は常にAPI経由
3. **透過的な移行準備**: 将来の物理分離を想定した設計
...
---
**この構成の適用条件**: 小規模チーム、初期開発段階、コスト制約環境

**推奨移行時期**: ユーザー数増加、パフォーマンス要件の変化、セキュリティ要件の高度化に応じて物理的分離を検討
