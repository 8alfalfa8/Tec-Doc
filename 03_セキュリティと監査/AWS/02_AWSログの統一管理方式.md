...
- [✅ 目的](#目的)
- [🔧 推奨システム構成（構成図付き）](#推奨システム構成構成図付き)
  - [① ログ収集・保存](#①-ログ収集保存)
  - [② ログ転送・整形](#②-ログ転送整形)
  - [③ ログ保存](#③-ログ保存)
  - [④ 可視化・ダッシュボード](#④-可視化ダッシュボード)
- [🖼️ 構成図（テキスト版）](#構成図テキスト版)
- [🎯 補足ポイント](#補足ポイント)
- [💡 シナリオ別構成例](#シナリオ別構成例)
- [📌 注意点・ベストプラクティス](#注意点ベストプラクティス)
- [✅ CloudWatch LogsとCloudTrail Logsの保存設計](#cloudwatch-logsとcloudtrail-logsの保存設計)
- [✅ 一括可視化のアプローチ（時系列で見たい）](#一括可視化のアプローチ時系列で見たい)
  - [① 共通の保存形式に整形する（ETL処理）](#①-共通の保存形式に整形するetl処理)
  - [② Athenaビューで統合](#②-athenaビューで統合)
  - [③ 可視化ツールで一括表示](#③-可視化ツールで一括表示)
- [🎯 注意点と工夫](#注意点と工夫)
- [✅ 統一分析のための保存構成例（S3パス）](#統一分析のための保存構成例s3パス)
- [🔧 オプション：ログ統合管理基盤にする場合](#オプションログ統合管理基盤にする場合)
- [✅ まとめ](#まとめ)
- [✅ CloudTrailログをCloudWatch Logsに出力する構成](#cloudtrailログをcloudwatch-logsに出力する構成)
  - [1. 出力設定（前提）](#1-出力設定前提)
- [✅ 構成イメージ](#構成イメージ)
- [✅ この構成のメリット・デメリット](#この構成のメリットデメリット)
- [✅ 分析・可視化の方法（CloudWatch内）](#分析可視化の方法cloudwatch内)
  - [1. CloudWatch Logs Insights（高度検索クエリ）](#1-cloudwatch-logs-insights高度検索クエリ)
  - [2. CloudWatch Dashboards](#2-cloudwatch-dashboards)
  - [3. Metric Filter + Alarm](#3-metric-filter-alarm)
- [✅ まとめ：CloudWatch集中案はこんな時におすすめ](#まとめcloudwatch集中案はこんな時におすすめ)
- [🔧 補足：CloudTrailのCloudWatch Logs出力設定方法](#補足cloudtrailのcloudwatch-logs出力設定方法)
- [✅ 目的](#目的)
- [✅ 方式①：S3 + Athena / QuickSight 連携構成（分析・レポート重視）](#方式①s3-athena-quicksight-連携構成分析レポート重視)
  - [🔄 構成の流れ](#構成の流れ)
  - [🖼️ 構成図（テキスト版）](#構成図テキスト版)
  - [✅ メリット / デメリット](#メリット-デメリット)
  - [🎯 この構成が適するケース](#この構成が適するケース)
- [✅ 方式②：CloudWatch Logs集中構成（即時モニタリング重視）](#方式②cloudwatch-logs集中構成即時モニタリング重視)
  - [🔄 構成の流れ](#構成の流れ)
  - [🖼️ 構成図（テキスト版）](#構成図テキスト版)
  - [✅ メリット / デメリット](#メリット-デメリット)
  - [🎯 この構成が適するケース](#この構成が適するケース)
- [✅ 両構成の比較まとめ（表）](#両構成の比較まとめ表)
- [🧭 どちらの構成が向いているか？](#どちらの構成が向いているか)
- [🔍 結論：別々に分析・可視化したいなら](#結論別々に分析可視化したいなら)
- [✅ おすすめ構成：**方式③:ハイブリッド型（両案の併用）**](#おすすめ構成方式③ハイブリッド型両案の併用)
  - [💡 システム構成概要](#システム構成概要)
  - [🖼️ 構成図（テキスト簡易版）](#構成図テキスト簡易版)
- [📌 まとめ](#まとめ)
- [✅ CloudTrailの出力先は2つ選べる](#cloudtrailの出力先は2つ選べる)
- [✅ 方式③における判断基準](#方式③における判断基準)
- [✅ 推奨構成（方式③）](#推奨構成方式③)
  - [📝 具体例：CloudTrailからDelete操作をリアルタイム監視したい](#具体例cloudtrailからdelete操作をリアルタイム監視したい)
- [✅ 結論](#結論)
- [✅ 両者のログ出力の違い](#両者のログ出力の違い)
- [✅ 重複の可能性について](#重複の可能性について)
- [✅ 統合分析時の注意点](#統合分析時の注意点)
  - [1. **イベントの重複除外処理をする**](#1-イベントの重複除外処理をする)
  - [2. **出力元で統合設計を行う**](#2-出力元で統合設計を行う)
  - [3. **GlueやLambdaで一元整形**](#3-glueやlambdaで一元整形)
- [✅ 結論まとめ](#結論まとめ)
<!-- TOC_END -->
