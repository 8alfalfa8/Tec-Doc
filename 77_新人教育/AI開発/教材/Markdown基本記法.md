...
基本的な記法として、見出しは「#」、箇条書きは「-」や「*」、強調は「**太字**」や「*斜体*」、リンクは「[表示名](URL)」の形式で記述します。
また、コードは「```」で囲むことで見やすく表現でき、エンジニア向けドキュメントとの相性が非常に良いです。
...
シンプルなテキストで複雑な図を描画できるため、AI開発においても、仕様書や設計書などのドキュメントに利用されています。
