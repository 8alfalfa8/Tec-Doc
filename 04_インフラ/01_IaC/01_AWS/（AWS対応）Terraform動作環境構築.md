...
- [✅ 構築ステップ概要](#構築ステップ概要)
- [🧰 1. Terraform CLIのインストール](#1-terraform-cliのインストール)
  - [ローカル環境（Mac / Windows / Linux）](#ローカル環境mac-windows-linux)
  - [EC2（Linux）上にTerraformをインストールする場合](#ec2linux上にterraformをインストールする場合)
- [🛡️ 2. AWS認証情報の準備（IAMユーザー＋アクセスキー）](#2-aws認証情報の準備iamユーザーアクセスキー)
- [📁 3. Terraform設定ファイルを作成（例）](#3-terraform設定ファイルを作成例)
- [▶ 4. Terraform実行手順](#4-terraform実行手順)
- [📦 5. （推奨）ステート管理のリモート化（S3＋DynamoDB）](#5-推奨ステート管理のリモート化s3dynamodb)
- [✅ まとめ：最小構成で始めるには](#まとめ最小構成で始めるには)
