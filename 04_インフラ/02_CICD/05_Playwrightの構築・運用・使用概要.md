...
- [1. Playwrightとは（位置づけ）](#1-playwrightとは位置づけ)
  - [1.1 概要](#11-概要)
  - [1.2 他ツールとの比較（簡易）](#12-他ツールとの比較簡易)
- [2. 標準アーキテクチャ（推奨）](#2-標準アーキテクチャ推奨)
- [3. Playwright 構築手順（Linux）](#3-playwright-構築手順linux)
  - [3.1 前提条件](#31-前提条件)
  - [3.2 Node.js インストール（例）](#32-nodejs-インストール例)
  - [3.3 Playwright 初期化](#33-playwright-初期化)
    - [初期化時の選択（推奨）](#初期化時の選択推奨)
  - [3.4 ディレクトリ構成（標準）](#34-ディレクトリ構成標準)
- [4. Playwright 設定（重要）](#4-playwright-設定重要)
  - [4.1 playwright.config.ts（標準例）](#41-playwrightconfigts標準例)
- [5. テスト実装方法](#5-テスト実装方法)
  - [5.1 基本テスト例](#51-基本テスト例)
  - [5.2 金融・公共向けで重要な観点](#52-金融公共向けで重要な観点)
- [6. 運用設計（非常に重要）](#6-運用設計非常に重要)
  - [6.1 運用ポリシー](#61-運用ポリシー)
  - [6.2 テスト分類](#62-テスト分類)
- [7. CI/CD 連携](#7-cicd-連携)
  - [7.1 GitLab CI 例](#71-gitlab-ci-例)
  - [7.2 Jenkins 例（Declarative）](#72-jenkins-例declarative)
- [8. 障害対応・トラブルシュート](#8-障害対応トラブルシュート)
- [9. セキュリティ・監査対応](#9-セキュリティ監査対応)
- [10. 成果物一覧（プロジェクト納品レベル）](#10-成果物一覧プロジェクト納品レベル)
- [1. 目的](#1-目的)
- [2. 適用範囲](#2-適用範囲)
- [3. 試験方針（金融・公共向け）](#3-試験方針金融公共向け)
  - [3.1 基本方針](#31-基本方針)
  - [3.2 禁止事項](#32-禁止事項)
- [4. 試験レベルと位置づけ](#4-試験レベルと位置づけ)
- [5. 試験対象業務シナリオ](#5-試験対象業務シナリオ)
  - [5.1 代表的業務シナリオ例](#51-代表的業務シナリオ例)
- [6. 試験観点（重要）](#6-試験観点重要)
  - [6.1 業務観点](#61-業務観点)
  - [6.2 非機能観点](#62-非機能観点)
- [7. テストデータ設計](#7-テストデータ設計)
  - [7.1 テストデータ方針](#71-テストデータ方針)
  - [7.2 データ管理](#72-データ管理)
- [8. 実行環境](#8-実行環境)
- [9. 証跡・成果物管理（監査対応）](#9-証跡成果物管理監査対応)
  - [9.1 取得対象](#91-取得対象)
  - [9.2 保管方針](#92-保管方針)
- [10. 合否判定基準](#10-合否判定基準)
- [11. 障害対応・是正プロセス](#11-障害対応是正プロセス)
- [12. 体制・役割](#12-体制役割)
- [13. リスクと対策](#13-リスクと対策)
- [14. 付録](#14-付録)
- [1. 目的](#1-目的)
- [2. Flakyの定義（公式定義）](#2-flakyの定義公式定義)
- [3. 基本原則（金融・公共向け三原則）](#3-基本原則金融公共向け三原則)
- [4. 設計段階ルール（最重要）](#4-設計段階ルール最重要)
  - [4.1 テスト設計ルール](#41-テスト設計ルール)
  - [4.2 Locator設計ルール](#42-locator設計ルール)
- [5. 実装ルール（厳格）](#5-実装ルール厳格)
  - [5.1 禁止事項](#51-禁止事項)
  - [5.2 必須実装](#52-必須実装)
- [6. 環境・データ起因Flaky対策](#6-環境データ起因flaky対策)
  - [6.1 テストデータルール](#61-テストデータルール)
  - [6.2 環境差異対策](#62-環境差異対策)
- [7. retries運用ルール（重要）](#7-retries運用ルール重要)
- [8. Flaky発生時の是正プロセス（必須）](#8-flaky発生時の是正プロセス必須)
- [9. 原因分類テンプレート](#9-原因分類テンプレート)
- [10. 合否判定ルール（監査対応）](#10-合否判定ルール監査対応)
- [11. KPI・可視化](#11-kpi可視化)
- [12. レビュー・監査対応](#12-レビュー監査対応)
- [13. 付録](#13-付録)
<!-- TOC_END -->
