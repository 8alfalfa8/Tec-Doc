...
- [1. 目的](#1-目的)
- [2. 適用範囲](#2-適用範囲)
- [3. 設計方針（高レベル）](#3-設計方針高レベル)
- [4. 脅威モデル（代表）](#4-脅威モデル代表)
- [5. 推奨アーキテクチャ（方式） — 要約](#5-推奨アーキテクチャ方式-要約)
- [6. セッションライフサイクル設計](#6-セッションライフサイクル設計)
  - [6.1 発行（ログイン）](#61-発行ログイン)
  - [6.2 維持（リクエスト時）](#62-維持リクエスト時)
  - [6.3 更新（リフレッシュ）](#63-更新リフレッシュ)
  - [6.4 破棄（ログアウト / 失効）](#64-破棄ログアウト-失効)
- [7. トークン設計詳細](#7-トークン設計詳細)
  - [7.1 セッションID（サーバー状態管理）](#71-セッションidサーバー状態管理)
  - [7.2 JWT（ステートレス認証）設計（必要なら）](#72-jwtステートレス認証設計必要なら)
- [8. クッキー/保管のベストプラクティス](#8-クッキー保管のベストプラクティス)
- [9. CSRF対策](#9-csrf対策)
- [10. XSS対策](#10-xss対策)
- [11. セッション固定（Fixation）対策](#11-セッション固定fixation対策)
- [12. 多端末・多デバイス運用](#12-多端末多デバイス運用)
- [13. スケーラビリティと可用性](#13-スケーラビリティと可用性)
- [14. 監査・ログ・モニタリング](#14-監査ログモニタリング)
- [15. 運用（鍵管理・ローテーション）](#15-運用鍵管理ローテーション)
- [16. 退避・失効（ログアウト、パスワード変更時）](#16-退避失効ログアウトパスワード変更時)
- [17. API エンドポイント（例）](#17-api-エンドポイント例)
- [18. データモデル（例：Redis ハッシュ / DB テーブル）](#18-データモデル例redis-ハッシュ-db-テーブル)
  - [Redis（session:{id}）](#redissessionid)
  - [RDBMS テーブル（optional）](#rdbms-テーブルoptional)
- [19. チェックリスト（設計→実装→テスト）](#19-チェックリスト設計実装テスト)
  - [設計段階](#設計段階)
  - [実装段階](#実装段階)
  - [テスト段階](#テスト段階)
- [20. 例：SPA 用推奨フロー（簡潔）](#20-例spa-用推奨フロー簡潔)
- [21. 追加考慮点（業務要件に応じて）](#21-追加考慮点業務要件に応じて)
- [22. 運用手順（インシデント時）](#22-運用手順インシデント時)
- [23. 参考（実務的な値・推奨）](#23-参考実務的な値推奨)
- [24. 実装スニペット（概念・擬似コード）](#24-実装スニペット概念擬似コード)
- [25. まとめ（重要ポイント）](#25-まとめ重要ポイント)
<!-- TOC_END -->
